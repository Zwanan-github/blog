  ...
  return (
    <div className='h-full w-full'>
          ...
          <div className='space-y-2 w-full'>
            {messages.map((message, index) => (
              ...
              >
                <div
                  className={cn(
                    'flex flex-col max-w-full sm:max-w-[90%] md:max-w-[80%]',
                    message.role === 'user' ? 'items-end' : 'items-start',
                  )}
                >
                  <p className='px-2 py-1 text-sm font-mono w-fit'>
                    {message.role}
                  </p>
                  <MDComponents
                    hideCatalog={true}
                    content={message.content}
                    className={cn('rounded-xl p-2 shadow-sm')}
                  />
                </div>
              </div>
            ))}
            {loading && (
              <div className={cn('w-full flex justify-start')}>
                <div
                  className={cn(
                    'flex flex-col max-w-full sm:max-w-[90%] md:max-w-[80%] items-start',
                  )}
                >
          ...
          {messages.length == 0 && (
            <div className='flex flex-col items-center justify-center h-full w-full'>
              ...
              <Button
                ...
                onClick={() => setInput('列举可以使用的工具，以及作用')}
              >
                {'列举可以使用的工具，以及作用'}
              </Button>
              <Button
      ...
      <div className='flex items-center justify-center h-16 py-4'>
        ...
        />
        <Button className='ml-1' size='icon' onClick={sendMessage}>
          <SendIcon />
        </Button>
      </div>
    </div>
